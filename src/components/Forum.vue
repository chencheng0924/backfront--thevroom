        ...
        <div class="flex flex-col items-center">
            <div class="w-11/12 flex flex-col justify-center">
                <div class="flex text-xl w-full justify-around items-center mb-3">
                    <div class="w-11/12 flex justify-center" style="border: 1px solid #000">檢舉編號</div>
                    <div class="w-11/12 flex justify-center" style="border: 1px solid #000">文章編號</div>
                    <div class="w-11/12 flex justify-center" style="border: 1px solid #000">會員名稱</div>
                    <div class="w-11/12 flex justify-center" style="border: 1px solid #000">留言時間</div>
                ...
                <div class="w-full flex text-lg justify-around items-center my-5" v-for="forum in reply" :key="forum.article">
                    <div class="w-auto flex justify-center">{{forum[0]}}</div>
                    <div class="w-auto flex justify-center">{{forum[1]}}</div>
                    <div class="w-auto flex justify-left">{{forum[2]}}</div>
                    <div class="w-auto flex justify-center">{{forum[3]}}</div>
                    <div class="w-auto flex justify-center">{{forum[4]}}</div>
                    <div class="w-auto flex justify-center">{{forum[5]}}</div>
                    <!-- <div style="border: 1px solid #000" class="p-2">{{ forum.state }}</div> -->
                    <div>  <button class="btn w-[80px] h-[40px] bg-red-600 text-white rounded-xl" @click="toggle(forum[0], $event)" :class="{change: arr.includes(forum[0], $event)}">OFF</button></div>
                    <div><button @click="gogo(forum[0])" class="px-2 py-3 rounded-lg bg-black text-white">檢視</button></div>
                </div>
            </div>
            <div></div>
        </div>
