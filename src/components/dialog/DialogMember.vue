<template>
  <div style="min-width:600px">
    ...
    <div class="text-lg mb-10">
      <div>
        留言時間 :
        <input type="text" class="border-black border rounded-md" v-model="four">
      </div>
    </div>
    ...
    <div class="text-lg mb-10">
      <div>
        留言內容 : 
        <textarea cols="50" rows="5" class="border-black border rounded-md" placeholder="請輸入內容" v-model="five"></textarea>
      </div>
    </div>

    <div class="flex w-full justify-end text-lg mb-3">
        <!-- <button @click="gogoro2" class="border px-3 py-1">確認</button> -->
        <div class="bg-blue-600 ml-5"><button class="px-3 py-1 text-white" @click="gogoro2">確認</button></div>
    </div>
  </div>
</template>
...
export default {
    ...
    async created () {
      ...
      this.three = this.testtest[0][0];
      this.four = this.testtest[0][5];
      this.five = this.testtest[0][4]
    },
    data() {
        return {
            close: false,
